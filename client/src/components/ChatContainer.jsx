...
const handleSendMessage = async (e) => {
  ...
  const sent = await sendMessage({text: input.trim()});
  if (sent) setInput("")
}

// Handle sending an imag
const handleSendImage = async (e) => {
  ...
  reader.onloadend = async () => {
    const sent = await sendMessage({image: reader.result})
    if (sent) e.target.value = ""
  }
      ...
      <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6'>
        {messages.map((msg, index)=>(
          <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !== authUser._id && 'flex-row-reverse'}`}>
             ...
             <div className='text-center test-xs'>
        ...
        <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3 '>
          <div className='flex-1 flex items-center bg-gray-100/12 px-3 rounded-full mt-3'>
